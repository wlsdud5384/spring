	...
	public List selectAllArticles() {
		...
		try {
			...
			String query = "SELECT CASE WHEN LEVEL -1 > 0 then CONCAT(CONCAT(REPEAT('', level-1), '<'), board.title) "
					+ "ELSE board.title end as title, board.articleNO, board.parentNO, result.level, board.content, board.id, board.writeDate"
	                 + "   FROM"
		...
		public void insertNewArticle(sec03.brd02.ArticleVO article) {
			try {
				conn = dataFactory.getConnection();
